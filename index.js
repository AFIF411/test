...
(function() {
    ...
    // --- 1. Générer les bougies en arrière-plan ---
    function createCandles() {
        ...
        for (let i = 0; i < count; i++) {
            ...
            // Mélanger bougies et quelques gâteaux pour la fête
    ...
    function setupGiftInteraction() {
        ...
        function openGift(event) {
            if (isOpen) {
                // Si déjà ouvert, un petit rebond
    ...
    // --- 3. Créer des cœurs flottants au moment de l'ouverture ---
    function createFloatingHearts() {
        ...
        for (let i = 0; i < count; i++) {
            ...
            el.style.transition = 'all 2.8s cubic-bezier(0.34, 1.56, 0.64, 1)';
            el.style.transform = 'scale(0.3) rotate(0deg)';
            el.style.textShadow = '0 0 30px rgba(255,100,100,0.5)';
            document.body.appendChild(el);

            // Animation
            requestAnimationFrame(() => {
                el.style.transform = `translateY(-${80 + Math.random() * 100}px) scale(1.6) rotate(${Math.random() * 60 - 30}deg)`;
                el.style.opacity = '0';
            });
    ...
    // --- 5. (Optionnel) Gestion du redimensionnement ---
    // On ne fait rien de spécial, les bougies s'adaptent déjà en CSS
